 ...
 * Paper: Submitted to VLDB 2026
 Loop over W and k Hyperparameter tuning Modified by Solmaz Seyed Monir - W and k search loop for DAPG evaluation
 Dataset default 
 Graph construction Modified by Solmaz Seyed Monir - DAPG percentile-based pruning enabled
 algNameStream Logging pruning threshold Modified by Solmaz Seyed Monir - Algorithm name includes DAPG pruning threshold
 uniqueResults map Modified by Solmaz Seyed Monir - Multi-metric logging: recall, cost, time, pruning
 Summary log Modified by Solmaz Seyed Monir - Logging index size, time, recall, pruning for DAPG
 Time-stamped run end Modified by Solmaz Seyed Monir - Timestamped summary for batch run logging
 */
...
// [Modified by Solmaz Seyed Monir - DAPG integration - University of Washington
  //  W and k Value Loops]
	...
	for (float W : W_values) {
      ...
      // [Modified by Solmaz Seyed Monir - Custom default dataset config for DAPG
      // LSH-APG Configuration Adjustment for DAP]
...
//std::string data_fold = "/mnt/c/Users/Soli1/Solmaz/LSH-APG/cppCode/LSH-APG/", index_fold = "./indexes/";

			#else
			
//std::string data_fold = "E:/Dataset_for_c/", index_fold = data_fold + "graphIndex/";
		//		std::string data_fold = "C:/Users/Soli1/Solmaz/LSH-APG/cppCode/LSH-APG/", index_fold = "./indexes/";

			//std::string data_fold = "C:/Users/Soli1/Solmaz/LSH-APG/cppCode/LSH-APG/dataset/", index_fold = "./indexes/";
			std::string data_fold = "/home/solmaz/", index_fold = "./indexes/";
			#endif
        ...
        // [Modified by Solmaz Seyed Monir - DAPG percentile-based pruning enabled
        // DAPG Graph Construction
        // This is the main integration of DAP pruning into graph building]
...
// [Modified by Solmaz Seyed Monir - Algorithm name includes DAPG pruning threshold - University of Washington
...
algNameStream << "DAP_k" << k << "_th" << std::fixed << std::setprecision(3) << divG->last_threshold;
      ...
      // [Modified by Solmaz Seyed Monir - Multi-metric logging: recall, cost, time, pruning
